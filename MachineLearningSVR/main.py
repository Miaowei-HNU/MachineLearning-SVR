...
#测试
rbf_target_predict  = svr_rbf.predict(test_data)
...
#计算绝对误差 MAE
rbf_MAE=mean_absolute_error(test_target, rbf_target_predict)
lin_MAE=mean_absolute_error(test_target, lin_target_predict)

#计算均方误差 MSE
rbf_MSE=mean_squared_error(test_target, rbf_target_predict)
lin_MSE=mean_squared_error(test_target, lin_target_predict)

#计算均方根误差 RMSE
rbf_RMSE=np.sqrt(mean_squared_error(test_target,rbf_target_predict))
...
save_result['rbf模型预测值']=rbf_target_predict
save_result['liner模型预测值']=lin_target_predict
save_result.to_excel('./result/predict.xlsx')

#开始画图
plt.scatter(x,rbf_target_predict,alpha=0.8,c='r',label='rbf模型预测值')
plt.scatter(x,lin_target_predict,alpha=0.8,c='y',label='liner模型预测值')
...
print("rbf模型绝对误差MAE={}".format(rbf_MAE))
print("rbf模型绝对误差MSE={}".format(lin_MSE))
print("rbf模型绝对误差RMSE={}".format(rbf_RMSE))
print("liner模型绝对误差MAE={}".format(lin_MAE))
print("liner模型绝对误差MSE={}".format(rbf_MSE))
print("liner模型绝对误差RMSE={}".format(lin_RMSE))